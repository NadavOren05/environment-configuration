...
# Check if a GPG key with the name "digitalidf" already exists
if gpg --list-keys | grep -q "digitalidf"; then
    echo "GPG key for 'digitalidf' already exists. Skipping key generation."
else
    gpg --batch --passphrase '' --quick-gen-key digitalidf default default
